...
# ============================================================================
# FSDP2 Checkpointing Functions (using Distributed Checkpointing - DCP)
# ============================================================================
...
def save_fsdp2_checkpoint(
    ...
    """
    Save FSDP2 model and optimizer checkpoint using Distributed Checkpointing (DCP).
    
    FSDP2 uses fully_shard() which doesn't require FSDP.state_dict_type context.
    Each GPU saves its sharded parameters directly to disk, avoiding OOM.
    
    Args:
        model: FSDP2 model (wrapped with fully_shard)
        optimizer: Optimizer
    ...
    if rank == 0:
        print(f"Saving FSDP2 checkpoint to {checkpoint_path}")
    
    # Create DCP writer
    writer = dist_cp.FileSystemWriter(checkpoint_path)
    
    # Get state dicts - FSDP2 automatically handles sharding
    # No need for FSDP.state_dict_type context manager
    state_dict = {
    ...
    # Save using DCP - each GPU saves its shard in parallel
    t0 = time.perf_counter()
    dist_cp.save_state_dict(
        state_dict=state_dict,
    ...
    dist.barrier()  # Ensure all ranks finish saving
    t1 = time.perf_counter()
    
    if rank == 0:
        print(f"FSDP2 checkpoint saved to {checkpoint_path} in {t1-t0:.2f} seconds")
...
def load_fsdp2_checkpoint(
    ...
    """
    Load FSDP2 model and optimizer checkpoint using Distributed Checkpointing (DCP).
    
    Each GPU loads its corresponding shard directly from disk.
    ...
    Returns:
        epoch: Epoch number from checkpoint, or None if checkpoint not found
    """
    checkpoint_path = Path(checkpoint_dir)
    
    if not checkpoint_path.exists():
        if rank == 0:
            print(f"No checkpoint directory found at {checkpoint_path}, skipping load")
        return None
    
    if rank == 0:
        print(f"Loading FSDP2 checkpoint from {checkpoint_path}")
    ...
    # Load using DCP - each GPU loads its shard in parallel
    t0 = time.perf_counter()
    dist_cp.load_state_dict(
        state_dict=state_dict,
        storage_reader=reader,
    ...
    dist.barrier()  # Ensure all ranks finish loading
    t1 = time.perf_counter()
    ...
    if rank == 0:
        print(f"FSDP2 checkpoint loaded from {checkpoint_path} in {t1-t0:.2f} seconds")
    
    # Try to extract epoch number from checkpoint path
    # Format: checkpoint_epoch_{epoch}
    try:
