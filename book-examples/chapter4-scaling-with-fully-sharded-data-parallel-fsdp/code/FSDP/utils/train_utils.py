...
def setup():
    ...
    # Get local_rank to specify device_id and avoid barrier() warnings
...
def train(args, model, rank, world_size, train_loader, optimizer, epoch, sampler=None, is_last_epoch=False):
    model.train()
    ...
    for iteration, batch in enumerate(train_loader):
        ...
        if is_last_epoch and iteration == total_iterations - 1 and rank == 0:
            if torch.cuda.is_available():
                ...
                max_allocated_gb = torch.cuda.max_memory_allocated(local_rank) / (1024**3)
                print(f"\n{'='*70}")
